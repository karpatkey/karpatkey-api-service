    ...
    try {
        ...
        blogPosts?.data?.projectFeed?.posts.forEach(async (post: any) => {
            ...
            const data = {
                ...
                mirrorpostcreators: ('leafilipo.eth' === post.publisher.member.ens) ? `${post.publisher.member.displayName} | rsousamarques` : post.publisher.member.displayName,
            }
            const result = await postCollectionItem(config.wfCollectionID, config.wfAPIKey, data)
