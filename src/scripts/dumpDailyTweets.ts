...
(async () => {
    try {
        ...
        const tweets = (await getUserTweets(startTime, endTime))
            .sort((a: any, b: any) => {
        ...
        for (const tweet of tweets) {
            const tweetExists = collectionItems?.items?.find((item: any) => item?.twitterid === tweet?.id || item?.twitterid === tweet?.conversation_id)
            if (tweetExists) {
            ...
            if (tweetIndex !== -1) {
                ...
                tweetsToDump[tweetIndex].twitterpostcontenthtml = tweetsToDump[tweetIndex].twitterpostcontenthtml + '\n\n' + converter.makeHtml(text)
            } else {
                const data = {
                    name: tweet.id,
                    slug: tweet.id,
